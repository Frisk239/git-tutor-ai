...
export function getToolsDefinition(options?: {
  ...
  if (options?.categories && options.categories.length > 0) {
    filteredTools = filteredTools.filter((t) => options.categories!.includes(t.category));
  }
  ...
  // 按类别分组
  const byCategory = filteredTools.reduce(
    (acc, tool) => {
...
export function generateToolPrompt(): string {
  ...
  const sections: string[] = ['# 可用工具', '', '你可以使用以下工具来完成任务：', ''];
