...
import { focusChainTool, type TaskProgressItem, FocusChainManager } from './focus-chain.js';
...
- [ ] 带信息的任务 (重要)`;
  ...
  describe('边界情况', () => {
    it('应该处理包含额外信息的任务', async () => {
      const items = [{ title: '任务1', completed: true, info: '已完成' }];
    ...
    it('应该处理空标题的任务', async () => {
      const items = [{ title: '   ', completed: false }];
