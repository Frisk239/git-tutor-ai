...
export class WebSearchManager {
  ...
  constructor(config: WebSearchConfig = {}) {
    ...
    this.defaultProvider =
      config.defaultProvider || SearchProviderType.DUCKDUCKGO;
  ...
  private initializeProviders(config: WebSearchConfig): void {
    ...
    if (config.bing?.apiKey) {
      this.providers.set(
        ...
        new BingSearchProvider(config.bing)
      );
    ...
    if (config.google?.apiKey && config.google?.searchEngineId) {
      this.providers.set(
        ...
        new GoogleSearchProvider(config.google)
      );
    ...
    if (config.tavily?.apiKey) {
      this.providers.set(
        ...
        new TavilySearchProvider(config.tavily)
      );
  ...
  async search(
    ...
    if (!searchProvider) {
      throw new Error(
        `Search provider ${providerType} is not available. Please configure it.`
      );
    }
    ...
    if (!searchProvider.isAvailable()) {
      // 尝试使用备用提供商
      this.logger.warn(
        `Provider ${providerType} is not available, trying fallback`
      );
  ...
  private async searchWithFallback(
    ...
    throw new Error(
      "No search provider is available. Please configure at least one provider."
    );
  }
  ...
  if (!globalSearchManager) {
    ...
    if (process.env.GOOGLE_SEARCH_API_KEY && process.env.GOOGLE_SEARCH_ENGINE_ID) {
      config.google = {
        ...
        endpoint: process.env.GOOGLE_SEARCH_ENDPOINT || "https://customsearch.googleapis.com/customsearch/v1",
      };
      if (!config.defaultProvider) {
